...
# Tweak 5: Alias for interpretability
NONCLOSURE_WEIGHT = "eta"  # Î· is the non-closure weight, not just a coefficient
...
class KneeEvent:
    ...
    Tweak 2: Now tied to admissibility (Îº â‰¤ 0 or Î›_Ïƒ â†’ 1), not just slopes.
    """
    step: int
    metric: str  # "action", "lambda_E", "admissibility"
    value_before: float
    value_after: float
    slope_change: float
    significance: float
    # New: admissibility-based detection
    kappa_crossed_zero: bool = False
    lambda_sigma_near_one: bool = False
...
class FullChainState:
    ...
    # Î› - Tweak 1: history-relative
    lambda_E: float
    lambda_sigma: float
    lambda_excess: float  # NEW: Î› above minimum achievable
    bottleneck: str
    
    # Tweak 3: G-limited unreorganized
    g_limited_residual: float
...
def detect_knee(
    ...
    """
    Detect a knee (sharp transition).
    
    Tweak 2: Now also checks admissibility (Îº â‰¤ 0, Î›_Ïƒ â†’ 1).
    """
    ...
    # Admissibility-based detection (Tweak 2)
    kappa_crossed = prev.kappa > 0 and current.kappa <= 0
    sigma_near_one = current.lambda_sigma > 0.95
    
    # If admissibility boundary crossed, that's a knee
    if kappa_crossed or sigma_near_one:
        return KneeEvent(
            step=current.step,
            metric="admissibility",
            value_before=prev.kappa,
            value_after=current.kappa,
            slope_change=prev.kappa - current.kappa,
            significance=float('inf'),  # Definitional knee
            kappa_crossed_zero=kappa_crossed,
            lambda_sigma_near_one=sigma_near_one
        )
    
    # Slope-based detection (original)
    if metric == "lambda_E":
    ...
    if significance > threshold:
        return KneeEvent(
            ...
            kappa_crossed_zero=False,
            lambda_sigma_near_one=False
        )
...
def run_full_chain_simulation(
    ...
    steps: int = 10,
    seed: int = 42,
    reorg_budget: float = 5.0,  # G-like: reorganization throughput limit
    hbar_unit: int = 1,         # â„-like: minimum distinction quantum
    ignore_capacity_limits: bool = False,  # Tweak 4: counterfactual mode
    verbose: bool = True
) -> List[FullChainState]:
    """
    ...
    Parameters:
      ...
      - ignore_capacity_limits: If True, run counterfactual with infinite capacity
    """
    # Tweak 4: Create counterfactual interfaces if requested
    if ignore_capacity_limits:
        original_interfaces = network.interfaces()
        # Create interfaces with 1000x capacity
        inf_interfaces = [
            Interface(
                name=itf.name,
                capacity=itf.capacity * 1000,  # Effectively infinite
                epsilon=itf.epsilon,
                eta=itf.eta
            )
            for itf in original_interfaces
        ]
        # Rebuild network with infinite capacity
        new_edges = [
            Edge(e.u, e.v, next(i for i in inf_interfaces if i.name == e.interface.name))
            for e in network.edges
        ]
        network = Network(nodes=network.nodes, edges=new_edges)
    ...
    mode_str = "INFINITE CAPACITY (counterfactual)" if ignore_capacity_limits else "FINITE CAPACITY"
    
    if verbose:
        print("\n" + "=" * 70)
        print(f"FULL CHAIN UNIFICATION SIMULATION - {mode_str}")
        print("=" * 70)
        print(f"\nParameters:")
        print(f"  Steps: {steps}")
        print(f"  â„-unit (demand quantum): {hbar_unit}")
        print(f"  G-budget (reorg limit): {reorg_budget}")
        print(f"  Capacity mode: {mode_str}")
    ...
    # Tweak 1: Track minimum Î› for computing excess
    min_lambda_so_far = 0.0
    
    if verbose:
        header = f"{'Step':>4} {'Ob':>8} {'Cost':>8} {'Action':>10} {'Î›_E':>8} {'Î›_ex':>6} {'Î›_Ïƒ':>6} {'Îº':>6} {'Event':>20}"
        print(f"\n{header}")
        print("-" * 90)
    ...
    for i, ob in enumerate(motif):
        ...
        if result is None:
            if verbose:
                print(f"{i:>4} {ob.source}â†’{ob.target:>4} {'BLOCKED':>8} {cumulative_action:>10.2f} "
                      f"{'â€”':>8} {'â€”':>6} {'â€”':>6} {'â€”':>6} {'SATURATION':>20}")
            break
        ...
        # Tweak 1: Compute lambda excess (what can't be discharged)
        if i == 0:
            min_lambda_so_far = lambda_E
        lambda_excess = lambda_E - min_lambda_so_far
        min_lambda_so_far = min(min_lambda_so_far, lambda_E)
        ...
        # Compute Îº
        kappa = composability_index(final_load, interfaces)
        ...
        state = FullChainState(
            ...
            g_limited_residual=g_limited_residual,
            kappa=kappa,
            distances=distances,
        ...
        if len(history) >= 4:
            ...
            if knee and (knee.step == i or knee.kappa_crossed_zero or knee.lambda_sigma_near_one):
                knee_event = knee
                if knee.kappa_crossed_zero:
                    event_str = "KNEE (Îºâ†’0)"
                elif knee.lambda_sigma_near_one:
                    event_str = "KNEE (Î›_Ïƒâ†’1)"
                else:
        ...
        if verbose:
            if not event_str:
                event_str = "â€”"
            # Tweak 3: Show G-limited if relevant
            g_str = f" [G:{g_limited_residual:.1f}]" if g_limited_residual > 0 else ""
            print(f"{i:>4} {ob.source}â†’{ob.target:>4} {state.routing_cost:>8.2f} "
                  f"{cumulative_action:>10.2f} {lambda_E:>8.2f} {lambda_excess:>6.2f} "
                  f"{lambda_sigma:>5.0%} {kappa:>6.2f} {event_str:>20}{g_str}")
    ...
    if history and verbose:
        ...
        print(f"  Î›_E = {final.lambda_E:.2f} (excess = {final.lambda_excess:.2f}, "
              f"Î›_Ïƒ = {final.lambda_sigma:.0%}, bottleneck = {final.bottleneck})")
        
        # Tweak 3: G-limited residual
        total_g_limited = sum(s.g_limited_residual for s in history)
        if total_g_limited > 0:
            print(f"  G-limited residual (unreorganized): {total_g_limited:.2f}")
        
        # Geometry
        print(f"\nGeometry evolution (d from {network.nodes[0]}):")
        initial_d = ', '.join(f'{n}:{history[0].distances.get(n, 0):.2f}' 
                              for n in network.nodes[1:4] if history[0].distances.get(n, float('inf')) < float('inf'))
        final_d = ', '.join(f'{n}:{final.distances.get(n, 0):.2f}' 
                            for n in network.nodes[1:4] if final.distances.get(n, float('inf')) < float('inf'))
        print(f"  Initial: {initial_d or 'all finite'}")
        print(f"  Final:   {final_d or 'all infinite (saturated)'}")
        ...
        if knee_events:
            ...
            for s in knee_events[:3]:
                k = s.knee
                knee_type = "Îºâ†’0" if k.kappa_crossed_zero else ("Î›_Ïƒâ†’1" if k.lambda_sigma_near_one else f"{k.significance:.1f}Ïƒ")
                print(f"  Step {k.step}: {k.metric} [{knee_type}]")
    
    return history


def run_counterfactual_comparison(network: Network, steps: int = 10, seed: int = 42, verbose: bool = True):
    """
    Tweak 4: Run same motif with finite vs infinite capacity.
    
    Shows: all structure disappears when enforceability is free.
    """
    if verbose:
        print("\n" + "=" * 70)
        print("COUNTERFACTUAL COMPARISON: FINITE vs INFINITE CAPACITY")
        print("=" * 70)
    
    # Run with finite capacity
    history_finite = run_full_chain_simulation(
        network, steps=steps, seed=seed, 
        ignore_capacity_limits=False, verbose=False
    )
    
    # Run with infinite capacity
    history_infinite = run_full_chain_simulation(
        network, steps=steps, seed=seed,
        ignore_capacity_limits=True, verbose=False
    )
    
    if verbose:
        print("\n--- Comparison ---")
        print(f"{'Metric':<25} {'Finite':>15} {'Infinite':>15} {'Ratio':>10}")
        print("-" * 70)
        
        if history_finite and history_infinite:
            f_final = history_finite[-1]
            i_final = history_infinite[-1]
            
            metrics = [
                ("Steps completed", len(history_finite), len(history_infinite)),
                ("Final action (time)", f_final.cumulative_action, i_final.cumulative_action),
                ("Final Î›_E", f_final.lambda_E, i_final.lambda_E),
                ("Final Î›_Ïƒ", f_final.lambda_sigma, i_final.lambda_sigma),
                ("Final Îº", f_final.kappa, i_final.kappa),
                ("Knee events", sum(1 for s in history_finite if s.knee), 
                               sum(1 for s in history_infinite if s.knee)),
            ]
            
            for name, f_val, i_val in metrics:
                if isinstance(f_val, float):
                    ratio = f_val / i_val if i_val > 1e-9 else float('inf')
                    print(f"{name:<25} {f_val:>15.2f} {i_val:>15.2f} {ratio:>10.2f}")
                else:
                    print(f"{name:<25} {f_val:>15} {i_val:>15}")
            
            print(f"""
KEY INSIGHT:
  With infinite capacity:
    - Î›_Ïƒ â†’ 0 (no saturation)
    - Îº stays positive (always composable)
    - No knees (no sharp transitions)
    - Geometry doesn't evolve (distances stay constant)
  
  ALL STRUCTURE DISAPPEARS WHEN ENFORCEABILITY IS FREE.
  
  This proves: QM, GR, and SM emerge from FINITE enforcement capacity.
""")
    
    return history_finite, history_infinite


def run_eta_comparison(network: Network, steps: int = 10, seed: int = 42, verbose: bool = True):
    """
    Compare different Î· (non-closure weight) values.
    
    Shows: lower Î· â†’ larger Regime R â†’ more classical behavior.
    """
    if verbose:
        print("\n" + "=" * 70)
        print("ETA (NON-CLOSURE WEIGHT) COMPARISON")
        print("=" * 70)
        print("\nÎ· controls quantum-like non-closure effects:")
        print("  Î· â†’ 0: Classical limit (large Regime R)")
        print("  Î· â†’ 1: Quantum limit (small Regime R, fast saturation)")
    ...
    for eta in eta_values:
        # Create interfaces with this eta
        interfaces = [
            Interface(f"I{i}", capacity=12.0, epsilon=1.0, eta=eta)
            for i in range(3)
        ]
        
        # Analyze regime
        regime = analyze_regime_boundary(interfaces, max_size=15)
        
        results.append({
            'eta': eta,
            'max_composable': regime['max_composable'],
            'composable_count': len(regime['composable'])
        })
    
    if verbose:
        print(f"\n{'Î·':>6} {'Max N*':>10} {'Regime R size':>15} {'Interpretation':>20}")
        print("-" * 55)
        
        for r in results:
            interp = "Classical" if r['max_composable'] > 8 else ("Quantum" if r['max_composable'] < 5 else "Intermediate")
            print(f"{r['eta']:>6.1f} {r['max_composable']:>10} {r['composable_count']:>15} {interp:>20}")
        
        print(f"""
INSIGHT:
  Lower Î· (non-closure weight) â†’ larger Regime R â†’ more classical behavior
  Higher Î· â†’ smaller Regime R â†’ faster quantum saturation
  
  This is why the classical limit (Î·â†’0) has no particle spectrum cutoff,
  while quantum systems (Î·>0) have finite particle content.
""")
...
""")
    ...
    # Tweak 4: Counterfactual comparison
    print("\n" + "=" * 70)
    print("COUNTERFACTUAL: WHAT IF CAPACITY WERE INFINITE?")
    print("=" * 70)
    
    history_finite, history_infinite = run_counterfactual_comparison(
        network, steps=10, seed=42, verbose=True
    )
    
    # Eta comparison
    run_eta_comparison(network, steps=10, seed=42, verbose=True)
